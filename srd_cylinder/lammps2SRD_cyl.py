...
# Cmd line args: poly.lammpstrj cylr cylh
...
print "lammpstrj cylr cylh"
infile = sys.argv[1]
npoly = 27
cylr = float(sys.argv[2])
cylh = float(sys.argv[3])

# calculate number of SRD particles:
cylvol = math.pi*cylr*cylr*cylh
nsrd = int(10*cylvol)
...
#    g.write("\t"+str(ctr)+" "+str(ctr)+str(" 2 0 "))
    rand_r = np.random.random_sample()
    rand_theta = np.random.random_sample()*2*math.pi
    x = math.sqrt(rand_r)*math.cos(rand_theta)*cylr
    y = math.sqrt(rand_r)*math.sin(rand_theta)*cylr
    z = np.random.random_sample()*cylh-cylh*0.5
