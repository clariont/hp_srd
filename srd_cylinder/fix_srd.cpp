...
FixSRD::FixSRD(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"lamda") == 0) {
      ...
      iarg += 2;
    // Start Modifications:
    } else if (strcmp(arg[iarg],"spc") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      spc = force->numeric(FLERR,arg[iarg+1]);
      srd_per_cell = spc;
      printf("read in: srd_per_cell: %lf\n", spc);
      iarg += 2;
    } else if (strcmp(arg[iarg],"cylr") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      cylr = force->numeric(FLERR,arg[iarg+1]);
      printf("read in: cylr %lf\n", cylr);
      iarg += 2;
    } else if (strcmp(arg[iarg],"force") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix srd command");
      clareForce = force->numeric(FLERR,arg[iarg+1]);
      iarg += 2;
    // End Modifications
...
void FixSRD::pre_neighbor()
{
  ...
  if (wallexist) {
    ...
    for (m = 0; m < nwall; m++) {
      ...
      // Clarion's edit - add wall to every bin
  ...
  if (bigexist || wallexist) {
    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        x[i][1] += dt_big*v[i][1];
//        x[i][2] += dt_big*v[i][2];
        x[i][2] += dt_big*(v[i][2]) + 0.5*dt_bigsq*clareForce;
	v[i][2] += dt_big*clareForce;
...
void FixSRD::reset_velocities()
{
  ...
  double cxlo, cxhi, cylo, cyhi;
  double cylrsq = cylr*cylr;
  double binr = sqrt(binsize1x*binsize1x+binsize1y*binsize1y);
  double dist_crit = cylr - binr;
  double dist_critsq = dist_crit*dist_crit+0.1;		// if bin position is greater than radius, check for overlaps:
  double binmidx, binmidy, binmidrsq;
  double cxhisq, cyhisq, cxlosq, cylosq;
  int intersect = 0;
  double mbexp = mass_srd/temperature_srd;
  ...
  for (i = 0; i < nbins; i++) {
    ...
    for (j = binhead[i]; j >= 0; j = binnext[j]) {
      // 1) Get Bin Coords:
      if (j == binhead[i]) {
	  ix = static_cast<int> ((x[j][0]-corner[0])*bininv1x);
	  ix = MAX(ix,binlo[0]);
	  ix = MIN(ix,binhi[0]);
	  iy = static_cast<int> ((x[j][1]-corner[1])*bininv1y);
	  iy = MAX(iy,binlo[1]);
	  iy = MIN(iy,binhi[1]);
	  cxlo = ix/bininv1x+corner[0];
	  cylo = iy/bininv1x+corner[1];
	  cxhi = cxlo+binsize1x;
	  cyhi = cylo+binsize1y;

	  binmidx = 0.5*(cxlo + cxhi);
	  binmidy = 0.5*(cylo + cyhi);
	  binmidrsq = binmidx*binmidx + binmidy*binmidy;

	  // 2) check if bin is within 2 bin-distances of the wall:
	  //     if so, see if it intersects the cylinder:
	  if (binmidrsq > dist_critsq) {
	      cxlosq = cxlo*cxlo;
	      cxhisq = cxhi*cxhi;
	      cylosq = cylo*cylo;
	      cyhisq = cyhi*cyhi;

	      // check the 4 bin edges to see if they intersect the cylinder wall;
	      if ((cxhisq+cyhisq) < cylrsq && (cxhisq+cylosq) > cylrsq)
		  intersect = 1;
	      if ((cxhisq+cyhisq) > cylrsq && (cxhisq+cylosq) < cylrsq)
		  intersect = 1;
	      if ((cxhisq+cyhisq) < cylrsq && (cxlosq+cyhisq) > cylrsq)
		  intersect = 1;
	      if ((cxhisq+cyhisq) > cylrsq && (cxlosq+cyhisq) < cylrsq)
		  intersect = 1;
	      if ((cxlosq+cyhisq) < cylrsq && (cxlosq+cylosq) > cylrsq)
		  intersect = 1;
	      if ((cxlosq+cyhisq) > cylrsq && (cxlosq+cylosq) < cylrsq)
		  intersect = 1;
	      if ((cxlosq+cylosq) < cylrsq && (cxhisq+cylosq) > cylrsq)
		  intersect = 1;
	      if ((cxlosq+cylosq) > cylrsq && (cxhisq+cylosq) < cylrsq)
		  intersect = 1;
	  }
    ...
    //*******************************************************************************************************//
    // Add in (10-n) gaussian random numbers (luijten wall thermostat)
    int avgbin = srd_per_cell;
    int n_virtual = (avgbin - n);
    if (intersect == 1) {
//	printf("intersecting wall!\n");
	if (n_virtual > 0) {
	    for (int i = 0; i < n_virtual; i++) {
    //	    vsum[0] += mbprefactor*pow(random->gaussian(),mbexp);
		vsum[0] += random->gaussian();
		vsum[1] += random->gaussian();
		vsum[2] += random->gaussian();
		n++;
	    }
	}
	else {
...
//        else inside = inside_wall(x[i],j);
        else inside = inside_cylinder(x[i]);
...
//              t_remain = collision_wall_exact(x[i],j,v[i],xscoll,xbcoll,norm);
            t_remain = collision_cyl_exact(x[i],j,v[i],xscoll,xbcoll,norm);
...
//              collision_wall_inexact(x[i],j,xscoll,xbcoll,norm);
              collision_cyl_inexact(x[i],j,xscoll,xbcoll,norm);
          }
          ...
          if (collidestyle == SLIP) {
            ...
            else noslip_cyl(v[i],NULL,x[j],big,j,xscoll,norm,vsnew);
          }
...
int FixSRD::inside_cylinder(double *xs)
{
//    printf ("called inside_cylinder\n");
//  double dx,dy,dz;
//
//  dx = xs[0] - xb[0];
//  dy = xs[1] - xb[1];
//  dz = xs[2] - xb[2];
  ...
  double y = xs[1];
  if ((x*x + y*y) > cylrsq) {
//      printf ("\tin inside_cylinder: %f %f %f\n", x, y, (x*x+y*y));
      return 1;
  }
...
double FixSRD::collision_cyl_exact(double *xs, int iwall, double *vs,
                                     double *xscoll, double *xbcoll,
                                     double *norm)
{
    double cylrsq = cylr*cylr;
...
//    printf("calling collision_cyl_exact\n");
  ...
  b = +2*(vs[0]*xs[0]+vs[1]*xs[1]);
  c = xs[0]*xs[0] + xs[1]*xs[1] + cylrsq;

  double dt = (-b + sqrt(b*b - 4*a*c)) / (2*a);
//  printf("a, b, c, dt: %lf %lf %lf %lf\n", a, b, c, dt);
//  printf("wall collision exact old, x,y,z: %f %f %f\n", xs[0], xs[1], xs[2]);
//  printf("wall collision exact old x^2 + y^2: %f\n", xs[0]*xs[0]+xs[1]*xs[1]);
  if (isnan(dt)) dt = 0.000001;

  xscoll[0] = xs[0] - dt*vs[0];
  xscoll[1] = xs[1] - dt*vs[1];
  xscoll[2] = xs[2] - dt*vs[2];
  
//  printf("wall collision exact, x,y,z: %f %f %f\n", xscoll[0], xscoll[1], xscoll[2]);

  // Calculate Norm:
    double myxsq = xs[0]*xs[0];
    double myysq = xs[1]*xs[1];
    double myzsq = xs[2]*xs[2];
//    double normscale = 1/sqrt(myxsq+myysq+myzsq);
    double scale = 1/sqrt(myxsq+myysq);
//    norm[0] = -xscoll[0]*normscale;
//    norm[1] = -xscoll[1]*normscale;
//    norm[2] = +xscoll[2]*normscale;
    norm[0] = -xscoll[0]*scale;
    norm[1] = -xscoll[1]*scale;
...
void FixSRD::collision_cyl_inexact(double *xs, int iwall, double *xscoll,
                                     double *xbcoll, double *norm)
// rescale SRD particle so that it sits on the surface of the cylinder.
{
//    printf("\t calling collision_cyl_inexact\n");
//  int dim = wallwhich[iwall] / 2;
//    double cylr = 8;
    double myx = xs[0];
    double myy = xs[1];
    double myz = xs[2];
    double myxsq = myx*myx;
    double myysq = myy*myy;
    double myzsq = myz*myz;
    double scale = 1/sqrt(myxsq+myysq);
    double normscale = 1/sqrt(myxsq+myysq+myzsq);

  // Place the SRD particle on the cylinder surface:
  xscoll[0] = myx*scale*cylr;
  xscoll[1] = myy*scale*cylr;
  xscoll[2] = myz;
//  xscoll[dim] = xwall[iwall];
//  printf("\twall collision, x,y,z, (x^2+y^2): %f %f %f %f\n", xscoll[0], xscoll[1], xscoll[2], xscoll[0]*xscoll[0] + xscoll[1]*xscoll[1]);

  // The normal vector to the cylinder wall points towards the center of the cylinder -> opposite of the SRD particle vector.
//  norm[0] = -myx*normscale;
//  norm[1] = -myy*normscale;
//  norm[2] = myz*normscale;
  norm[0] = -myx*scale;
  norm[1] = -myy*scale;
  norm[2] = 0;
...
void FixSRD::noslip_cyl(double *vs, double *vb, double *xb, Big *big, int iwall,
                    double *xsurf, double *norm, double *vsnew)
{
  double vs_dot_n,scale,r1,r2,vnmag,vtmag1,vtmag2;
  double tangent1[3],tangent2[3];
...
//  vsnew[2] = vnmag*norm[2] + vtmag1*tangent1[2] + vtmag2*tangent2[2];
  ...
  // Don't rescale the velocities and make them random - just reverse the velocities
  vsnew[0] = -vs[0];
  vsnew[1] = -vs[1];
  vsnew[2] = -vs[2];
